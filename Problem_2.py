...
class Solution:
    def shortestWay(self, source: str, target: str) -> int:
        #Approach 1
        #Use 2 pointers to iterate source and target, when pointer on source reaches end reset and increase count of subsequence by 1. Also store characters in source string in a hashSet if character in target not in source return -1
        # hashSet = set(source)
        # i, j = 0, 0
        # cnt = 1
        # while j < len(target):
